...
function App(){
 ...
 return <div className="site">
  <header className="nav"><div className="brand" onClick={()=>scroll('home')}><div className="brand-mark">HEB</div><div><strong>HAIDER</strong><span>ESTATE & BUILDERS</span><small>Building Spaces. Delivering Excellence.</small></div></div>
  ...
  <main>
   <section id="home" className="hero">
    ...
    <div className="hero-media"><video autoPlay muted loop playsInline poster={A+'construction-1.jpeg'}><source src={A+'under-construction.mp4'} type="video/mp4"/></video><div className="hero-overlay"></div><button className="play" onClick={()=>setVideo(videos[2])}><Play fill="currentColor"/></button><div className="hero-badge"><span>Your Expansion.</span><b>Our Execution.</b></div></div>
   </section>
   ...
   <section id="projects" className="section work"><div className="section-head"><div><p className="eyebrow">SELECTED WORK</p><h2>Featured <em>Projects.</em></h2></div><p>A selection of available project documentation showcasing our construction, structural and interior capabilities.</p></div><div className="project-grid">{projects.map((p,i)=><article className="project-card" key={p.title} onClick={()=>p.video&&setVideo(p)}><div className="project-img"><img src={A+p.image}/>{p.video&&<span className="video-pill"><Play size={13} fill="currentColor"/> VIDEO</span>}</div><div><h3>{p.title}</h3><p>{p.meta}</p><span>{p.desc}</span></div><ArrowUpRight size={19}/></article>)}</div></section>

   <section className="motion"><div className="motion-head"><div><p className="eyebrow">CONSTRUCTION IN MOTION</p><h2>See Our Work <em>in Action.</em></h2></div><button onClick={()=>scroll('projects')}>VIEW ALL PROJECTS <ArrowRight size={16}/></button></div><div className="video-grid">{videos.map(v=><button className="video-card" key={v.src} onClick={()=>setVideo(v)}><video muted preload="metadata" poster={A+'construction-1.jpeg'}><source src={A+v.src} type="video/mp4"/></video><span className="video-play"><Play size={19} fill="currentColor"/></span><div><h3>{v.title}</h3><p>{v.text}</p></div></button>)}</div></section>
   ...
   <section id="contact" className="contact section"><div className="contact-info"><p className="eyebrow">LET'S TALK</p><h2>Let's Build Your <em>Next Project.</em></h2><p>Tell us what you're planning. Our team will get in touch to discuss your requirements.</p><div className="details"><div><Phone size={19}/><span>Shamim Haider<br/><b>0300 3638688</b></span></div><div><Phone size={19}/><span>Inzamam Haider<br/><b>0312 2609099</b></span></div><div><MapPin size={19}/><span>Pakistan</span></div></div></div><form onSubmit={e=>{e.preventDefault();setSent(true)}}><div className="form-row"><input required placeholder="Full Name *"/><input placeholder="Company Name"/></div><div className="form-row"><input required type="tel" placeholder="Phone Number *"/><input required type="email" placeholder="Email Address *"/></div><div className="form-row"><select defaultValue=""><option value="" disabled>Project Type *</option><option>Restaurant / Outlet</option><option>Retail / Shop</option><option>Commercial Building</option><option>Bank / Institutional</option><option>Renovation</option><option>Other</option></select><input placeholder="Project Location"/></div><textarea required placeholder="Tell us about your project *" rows="5"></textarea><button className="primary" type="submit">{sent?'INQUIRY SENT ✓':'SEND INQUIRY'} <ArrowRight size={17}/></button></form></section>
  </main>
  <footer><div className="footer-brand"><div className="brand-mark">HEB</div><div><strong>HAIDER</strong><span>ESTATE & BUILDERS</span><small>Building Spaces. Delivering Excellence.</small></div></div><div><b>QUICK LINKS</b>{nav.map(n=><button key={n} onClick={()=>scroll(n.toLowerCase())}>{n}</button>)}</div><div><b>OUR SERVICES</b>{services.slice(0,5).map(([n])=><span key={n}>{n}</span>)}</div><div><b>CONTACT INFO</b><span>0300 3638688</span><span>0312 2609099</span><span>Pakistan</span></div></footer>
  <div className="copyright">© 2026 Haider Estate & Builders. All Rights Reserved.</div>
  {video&&<div className="modal" onClick={()=>setVideo(null)}><div className="modal-inner" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setVideo(null)}><X/></button><video controls autoPlay><source src={A+video.src} type="video/mp4"/></video><div><p className="eyebrow">WORK SHOWCASE</p><h2>{video.title}</h2><p>{video.text}</p></div></div></div>}
 </div>
}
